// ================================================================
// Airflow OMS — 검수 (1차 지역검수 / 2차 HQ검수) v7.0
// State Machine 적용 — DONE → REGION_APPROVED|REGION_REJECTED
// try-catch 강화: 검수 실패 시 비즈니스 컨텍스트 로깅
// ================================================================
...
export function mountReview(router: Hono<Env>) {
  ...
  router.post('/:order_id/review/region', async (c) => {
    ...
    try {
      ...
      const result = await transitionOrder(db, orderId, targetStatus, user, {
        ...
        afterTransition: async (db, order) => {
          ...
          try {
            ...
            console.error(`[review/region] 알림 전송 실패 order_id=${orderId}:`, notifErr.message);
            // 알림 실패는 검수 결과에 영향 없음 — 계속 진행
          }
        },
      });
      ...
      console.error(`[review/region] 지역 검수 실패 order_id=${orderId}:`, err.message);
      return c.json({ error: '검수 처리 중 오류가 발생했습니다.', code: 'REVIEW_ERROR' }, 500);
    }
  });
  ...
  router.post('/:order_id/review/hq', async (c) => {
    ...
    try {
      ...
      const result = await transitionOrder(db, orderId, targetStatus, user, {
        ...
        afterTransition: async (db, order) => {
          ...
          try {
            ...
            console.error(`[review/hq] 알림 전송 실패 order_id=${orderId}:`, notifErr.message);
          }
        },
      });
      ...
      console.error(`[review/hq] HQ 검수 실패 order_id=${orderId}:`, err.message);
      return c.json({ error: '검수 처리 중 오류가 발생했습니다.', code: 'REVIEW_ERROR' }, 500);
    }
  });
}
