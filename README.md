...
### E2E 통합 테스트 (50/50 통과)
- **`tests/e2e.sh`** — 386줄, 50개 테스트, 15개 영역 커버
- 헬스체크, 인증, 주문 CRUD, 주문 라이프사이클, 배치, 정산, 통계, 정책, 알림, 시스템, HR/감사, 채널/대리점, RBAC, 매출/정산차트, 로그아웃
- 3역할 교차 테스트: SUPER_ADMIN, REGION_ADMIN, TEAM_LEADER

### 버그 수정
- **로그아웃 API**: Cookie에서 session_id 미추출 → `getSessionCookie()` 헬퍼 추가
- **보고서 사진 업로드**: `photo.url` / `photo.file_url` 양쪽 수용

### 에러 핸들링 강화
- **프론트엔드**: API 재시도(GET 2회), 오프라인 감지+배너, 요청 타임아웃(30초), 글로벌 에러 핸들러
- **백엔드**: 에러 자동 분류(DB/검증/타임아웃/404), 에러 코드 표준화, `_debug` 필드
...
- **DONE**: 최종완료 (영수증 첨부)
- 15단계 상태 전이: RECEIVED→...→READY_DONE→IN_PROGRESS→SUBMITTED→DONE→REGION_APPROVED→HQ_APPROVED→...→PAID

### 정책관리 CRUD
- 배분/보고서/수수료 정책 생성·수정 7개 엔드포인트
- 통계 페이지 4탭 정책관리 UI

## 핵심 비즈니스 흐름
```
주문 수신 → 유효성검증 → 행정동 기반 자동배분 → 팀장 배정
→ 준비완료 → 작업 수행 → 보고서 제출 → 최종완료
→ 지역 1차 검수 → HQ 2차 검수
...
| 주문관리 | CRUD, 필터, 드로어 상세, 배치 액션, 일괄/개별 배분, CSV/xlsx 내보내기 |
...
| 내주문 | 팀장 전용, READY_DONE/DONE 플로우, 보고서 제출, 프로필/알림설정 |
...
총 **~100개** API 엔드포인트. 상세 맵은 `ARCHITECTURE.md` 참조.

| 도메인 | 경로 프리픽스 | 주요 기능 |
|--------|-------------|-----------|
| 인증 | /api/auth | 로그인, 로그아웃, 세션, 조직/팀장 조회 |
| 주문 | /api/orders | CRUD, 자동/수동/일괄배분, 배정, 준비완료, 작업시작, 보고서, 최종완료, 검수 |
...
| 시스템 | /api/system | 시스템정보, 세션관리, 검색, 타임라인, 임포트/백업/복원, 푸시 |
...
- **버전**: v16.0.0
- **총 코드량**: Backend 8,902 + Frontend 10,722 + SW 143 + CSS 419 + SQL 1,112 + E2E 386 = **21,684줄**
- **최종 업데이트**: 2026-03-05
