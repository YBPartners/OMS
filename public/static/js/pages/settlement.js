...
<div class="footer">Airflow | 생성일: ${generated_at} | 이 문서는 자동 생성되었습니다.</div>
</body></html>`;
...
async function exportSettlementCSV(runId) {
  ...
  const rows = res.rows.map(r => [
    ...
    r.channel_name || '', r.base_amount, r.commission_mode, r.commission_rate,
...
<div class="footer">Airflow | 생성일: ${today}</div></body></html>`;
...
<div class="inv-page${hasPageBreak ? ' page-break' : ''}">
  ...
  <div class="inv-meta">
    <div class="box">
      <div class="title">발행처 (Airflow)</div>
  ...
  <div class="inv-footer">Airflow | 본 계산서는 자동 생성되었습니다 | 발행일: ${inv.issueDate}</div>
</div>`;
}
