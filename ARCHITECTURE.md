...
│   │   └── validators.ts         # 입력 검증
│   ├── services/                 # ★ 서비스 레이어 (v6.5 신규)
│   │   ├── index.ts              # 전체 서비스 재수출
│   │   ├── notification-service.ts  # 알림 생성/배치 (유일 쓰기 진입점)
│   │   ├── session-service.ts    # 세션 CRUD/만료정리/무효화
...
│                    Routes (라우트)                     │
├──────────┬──────────┬──────────┬──────────┬──────────┤
│ auth.ts  │signup/   │hr/users  │settle/   │region-   │
│          │index.ts  │.ts       │calc.ts   │add.ts    │
└────┬─────┴────┬─────┴────┬─────┴────┬─────┴────┬─────┘
     │          │          │          │          │
     ▼          ▼          ▼          ▼          ▼
┌─────────┐┌─────────┐┌─────────┐┌──────────┐┌──────────┐
...
| 서비스 | 소유 테이블 (쓰기) | 호출자 |
|--------|-------------------|--------|
| notification-service | notifications | signup, region-add, (향후 orders, settlements) |
...
### 6.3 인터랙션 디자인 시스템 (interactions.js)
```
